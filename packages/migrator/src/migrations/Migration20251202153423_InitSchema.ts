...
export class Migration20251202153423_InitSchema extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "user" ("id" varchar(21) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" varchar(255) not null, "email" varchar(255) not null, "password" varchar(255) null, "email_verified" boolean not null default false, "plan" text check ("plan" in ('free', 'hobbyist', 'pro', 'enterprise')) not null default 'free', "max_tunnels" int not null default 1, "max_bandwidth_mb" int not null default 1000, "is_active" boolean not null default true, "auth_token" varchar(255) null, constraint "user_pkey" primary key ("id"));`
    );
    ...
    this.addSql(
      `create table "domain" ("id" varchar(21) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "user_id" varchar(21) not null, "hostname" varchar(255) not null, "is_reserved" boolean not null default false, constraint "domain_pkey" primary key ("id"));`
    );
