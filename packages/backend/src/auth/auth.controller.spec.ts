...
import { OAuthProvider, User, UserPlan } from '@noverlink/backend-shared';
  ...
  describe('Device Code Flow', () => {
    ...
    describe('pollDeviceFlow', () => {
      it('should poll for authorization status', async () => {
        authService.pollDeviceFlow.mockResolvedValue({ error: 'authorization_pending' });

        const result = await controller.pollDeviceFlow({ device_code: 'device-123' });
      ...
      it('should return auth token when approved', async () => {
        authService.pollDeviceFlow.mockResolvedValue({ auth_token: 'nv_token-123' });
    ...
    describe('approveDeviceCode', () => {
      it('should approve device code for authenticated user', async () => {
        ...
        const result = await controller.approveDeviceCode('ABCD-1234', mockUser as never);

        expect(result).toEqual({ success: true });
        expect(authService.approveDeviceCode).toHaveBeenCalledWith('ABCD-1234', 'user-123');
      });

      it('should return false for invalid code', async () => {
        authService.approveDeviceCode.mockResolvedValue(false);

        const result = await controller.approveDeviceCode('INVALID', mockUser as never);
