...
describe('TunnelsService', () => {
  ...
  describe('listSessions', () => {
    ...
    it('should return nextCursor when hasMore', async () => {
      const manySessions = Array(21).fill(null).map((_, i) => ({
        ...mockSessions[0],
        id: `session-${i}`,
      ...
      const result = await service.listSessions('user-123', undefined, undefined, 20);
  ...
  describe('getSessionLogs', () => {
    ...
    it('should filter by method when provided', async () => {
      ...
      await service.getSessionLogs('user-123', 'session-123', undefined, 50, 'get');
  ...
  describe('getSessionRequestCount', () => {
    it('should return request count for session', async () => {
      ...
      expect(em.count).toHaveBeenCalledWith(HttpRequest, { session: 'session-123' });
    });
  });
});
