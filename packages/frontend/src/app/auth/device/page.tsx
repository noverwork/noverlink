...
const deviceCodeSchema = z.object({
  userCode: z
    ...
    .regex(/^[A-Z0-9]{4}-[A-Z0-9]{4}$/, 'Invalid code format (e.g., ABCD-1234)'),
});
...
function DeviceAuthContent() {
  ...
  const onSubmit = async (data: DeviceCodeFormData) => {
    if (!isAuthenticated) {
      ...
      const returnUrl = `/auth/device?code=${encodeURIComponent(data.userCode)}`;
    ...
    try {
      ...
      if (result.success) {
        ...
        setErrorMessage('Failed to authorize. The code may be invalid or expired.');
        setStatus('error');
      }
    } catch (error) {
      const message =
          ...
          <Card className="bg-slate-900 border-white/[0.08]">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-sm text-slate-400 mb-6">
                Your CLI is now connected. You can close this window and return to
                your terminal.
              </p>
              <Link href="/dashboard">
          ...
          <CardContent className="space-y-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="space-y-2">
                ...
                />
                {errors.userCode && (
                  <p className="text-xs text-rose-400">{errors.userCode.message}</p>
                )}
              </div>
            ...
            <div className="pt-6 border-t border-slate-700">
              <p className="text-xs text-slate-500 text-center">
                Only authorize devices you trust. This will give the CLI access to
